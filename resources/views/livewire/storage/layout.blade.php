...
<body class="bg-light">
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-brand bg-brand-border" aria-label="Main navigation">
    <div class="container-xl">
      <a href="/{{ $lang }}/storage" class="navbar-brand">
        <img src="/img/365.png" alt="Cargo365" height="35">
      </a>
      ...
      <div class="navbar-collapse offcanvas-collapse" id="navbarsExampleDefault">
        <ul class="navbar-nav py-2 mx-auto-">
          ...
          <li class="nav-item">
            <a class="nav-link px-3 text-white" href="/{{ $lang }}/storage/tracks">All tracks</a>
          </li>
          @can('reception', Auth::user())
            <li class="nav-item">
              <a class="nav-link px-3 text-white" href="/{{ $lang }}/storage/reception">Reception</a>
            </li>
          @endcan
          @can('sending', Auth::user())
            <li class="nav-item">
              <a class="nav-link px-3 text-white" href="/{{ $lang }}/storage/sending">Sending</a>
            </li>
          @endcan
          @can('sorting', Auth::user())
            <li class="nav-item">
              <a class="nav-link px-3 text-white" href="/{{ $lang }}/storage/sorting">Sorting</a>
            </li>
          @endcan
          @can('arrival', Auth::user())
            <li class="nav-item">
              <a class="nav-link px-3 text-white" href="/{{ $lang }}/storage/arrival">Arrival</a>
            </li>
          @endcan
          @can('giving', Auth::user())
            <li class="nav-item">
              <a class="nav-link px-3 text-white" href="/{{ $lang }}/storage/giving">Giving</a>
            </li>
          @endcan
        </ul>
